...
color_filter!(OnBrightYellowFilter, OnBrightYellowFilterImpl, "on_bright_yellow", on_bright_yellow);
color_filter!(
    OnBrightRedFilter,
...
/// Serializes the input value as a JSON-encoded string (quoted and escaped).
#[derive(Clone)]
pub struct JsonFilter;

#[derive(Debug)]
struct JsonFilterImpl;

impl std::fmt::Display for JsonFilterImpl {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "json")
    }
}
