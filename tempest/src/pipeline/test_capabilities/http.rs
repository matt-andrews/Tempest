...
impl HttpTestCapability{
    pub fn new(route: &str, options: &OptionsModel, test: &TestModel) -> Self{
        Self{ route: route.to_string(), options: options.clone(), test: test.clone() }
    }
}
#[async_trait]
...
mod tests {
    ...
    fn test_model(route: &str, verb: Option<&str>) -> TestModel {
        TestModel { route: route.to_string(), verb: verb.map(str::to_string), ..Default::default() }
    }
    ...
    async fn get_returns_status_and_body() {
        ...
        let result = capability(&format!("{}/ping", server.url()), &test_model("/ping", Some("GET"))).test().await;
    ...
    async fn verb_defaults_to_get_when_not_set() {
        let mut server = Server::new_async().await;
        let mock = server.mock("GET", "/default")
            .with_status(200)
            .create_async().await;

        let result = capability(&format!("{}/default", server.url()), &test_model("/default", None)).test().await;
    ...
    async fn post_sends_body_and_receives_response() {
        let mut server = Server::new_async().await;
        let mock = server.mock("POST", "/submit")
        ...
        let result = capability(&format!("{}/submit", server.url()), &test).test().await;
    ...
    async fn put_sends_body() {
        let mut server = Server::new_async().await;
        let mock = server.mock("PUT", "/item/1")
            .match_body("updated")
            .with_status(200)
            .create_async().await;
        ...
        let result = capability(&format!("{}/item/1", server.url()), &test).test().await;
    ...
    async fn delete_returns_expected_status() {
        let mut server = Server::new_async().await;
        let mock = server.mock("DELETE", "/item/1")
            .with_status(204)
            .create_async().await;

        let result = capability(&format!("{}/item/1", server.url()), &test_model("/item/1", Some("DELETE"))).test().await;
    ...
    async fn unknown_verb_returns_default_result_without_sending_request() {
        ...
        assert_eq!(result.status.code, 0, "unknown verb should yield zero status, not a network attempt");
    ...
    async fn custom_request_headers_are_forwarded() {
        let mut server = Server::new_async().await;
        let mock = server.mock("GET", "/headers")
        ...
        let result = capability(&format!("{}/headers", server.url()), &test).test().await;
    ...
    async fn json_response_is_parsed_into_json_field() {
        let mut server = Server::new_async().await;
        server.mock("GET", "/data")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(r#"{"key":"value"}"#)
            .create_async().await;

        let result = capability(&format!("{}/data", server.url()), &test_model("/data", Some("GET"))).test().await;

        assert!(result.json.is_some(), "json field should be populated for valid JSON bodies");
    ...
    async fn non_json_response_leaves_json_field_none() {
        let mut server = Server::new_async().await;
        server.mock("GET", "/text")
            .with_status(200)
            .with_body("plain text")
            .create_async().await;

        let result = capability(&format!("{}/text", server.url()), &test_model("/text", Some("GET"))).test().await;
    ...
    async fn response_headers_are_captured() {
        ...
        let result = capability(&format!("{}/hdr", server.url()), &test_model("/hdr", Some("GET"))).test().await;
    ...
    async fn connection_error_produces_504_status() {
        ...
        let cap = HttpTestCapability::new("http://127.0.0.1:1/nope", &OptionsModel::default(), &test);
        let result = cap.test().await;
    ...
    async fn duration_is_measured_for_successful_requests() {
        let mut server = Server::new_async().await;
        server.mock("GET", "/slow").with_status(200).create_async().await;

        let result = capability(&format!("{}/slow", server.url()), &test_model("/slow", Some("GET"))).test().await;

        assert!(result.duration.as_nanos() > 0, "duration should be non-zero for a real HTTP round trip");
    }
}